...
public class StructureTemplateLoader {
    ...
    public static LoadedTemplate loadFromNBT(ServerLevel level, String structureName) {        File structuresDir = new File(System.getProperty("user.dir"), "structures");
        agentMod.LOGGER.info("Structures directory: {}", structuresDir.getAbsolutePath());
        
        File exactMatch = new File(structuresDir, structureName + ".nbt");
        if (exactMatch.exists()) {
            agentMod.LOGGER.info("Found structure (exact match): {}", exactMatch.getName());
        ...
        if (spacedMatch.exists()) {
            agentMod.LOGGER.info("Found structure (spaced match): {}", spacedMatch.getName());
        ...
        if (structuresDir.exists() && structuresDir.isDirectory()) {
            ...
            if (files != null && files.length > 0) {
                agentMod.LOGGER.info("Found structure (fuzzy match): {}", files[0].getName());
                return loadFromFile(files[0], structureName);
            }
        }
        
        try {
            ResourceLocation resourceLocation = new ResourceLocation("mc-agenticlabs", structureName);
        ...
        agentMod.LOGGER.warn("Structure '{}' not found. Available structures: {}", 
    ...
    private static LoadedTemplate loadFromFile(File file, String name) {
        try (InputStream inputStream = new FileInputStream(file)) {
            ...
            agentMod.LOGGER.error("Failed to load structure from file: {}", file, e);
            return null;
        }
    }
    ...
    private static LoadedTemplate loadFromMinecraftTemplate(StructureTemplate template, String name) {
        ...
        agentMod.LOGGER.warn("Direct template loading not fully implemented, please use NBT files directly");
        return null;
    }
    ...
    private static LoadedTemplate parseNBTStructure(CompoundTag nbt, String name) {
        ...
        for (int i = 0; i < paletteList.size(); i++) {
            ...
            try {
                ...
                agentMod.LOGGER.warn("Unknown block in structure: {}", blockName);
        ...
        agentMod.LOGGER.info("Loaded {} blocks from NBT '{}' ({}x{}x{})", blocks.size(), name, width, height, depth);
